        ...
        if(line[0] == 'f'){
            ...
            if(nLines == 3){
                ...
                if(!textured && !normaled){
                    ...
                    t2.t[0] = texs[stoi(tokens[v[2]+1]) - 1];
                    t2.t[1] = texs[stoi(tokens[v[3]+1]) - 1];
                    t2.t[2] = texs[stoi(tokens[v[0]+1]) - 1];
                    ...
                    t2.n[0] = m_normals[stoi(tokens[v[2]+2]) - 1];
                    t2.n[1] = m_normals[stoi(tokens[v[3]+2]) - 1];
                    t2.n[2] = m_normals[stoi(tokens[v[0]+2]) - 1];
...
int sr::Triangle_ClipAgainstPlane(sr::vec3 plane_p, sr::vec3 plane_n, sr::triangle &in_tri, sr::triangle &out_tri1, sr::triangle &out_tri2)
{
    ...
    if (nInsidePointCount == 1 && nOutsidePointCount == 2)
    {
        ...
        sr::vec3 n_p[3] = {*inside_points[0][0], *outside_points[0][0], *outside_points[1][0]};
        sr::vec3 n_n[3] = {*inside_points[0][1], *outside_points[0][1], *outside_points[1][1]};
    ...
    if (nInsidePointCount == 2 && nOutsidePointCount == 1)
    {
        ...
        sr::vec3 n_p1[3] = {*inside_points[0][0], *inside_points[1][0], *outside_points[0][0]};
        sr::vec3 n_n1[3] = {*inside_points[0][1], *inside_points[1][1], *outside_points[0][1]};

        sr::vec3 n_p2[3] = {*inside_points[1][0], out_tri1.p[2], *outside_points[0][0]};
        sr::vec3 n_n2[3] = {*inside_points[1][1], out_tri1.n[2], *outside_points[0][1]};
...
void sr::Raster(sr::ivec2 p[3], float depth[3], std::function<void(sr::ivec2, float, sr::vec3)> pixel, sr::internal_buffer_object *buffers, sr::ivec2 framebuffer_size){
    ...
    p[0].x = (p[0].x < 1 ? 2 : (p[0].x >= framebuffer_size.x-1 ? framebuffer_size.x-2 : p[0].x));
    p[1].x = (p[1].x < 1 ? 2 : (p[1].x >= framebuffer_size.x-1 ? framebuffer_size.x-2 : p[1].x));
    p[2].x = (p[2].x < 1 ? 2 : (p[2].x >= framebuffer_size.x-1 ? framebuffer_size.x-2 : p[2].x));

    std::function<void(int, int)> f = [y_min, y_max, &y_min_l, &y_max_l, buffers, &framebuffer_size](int x, int y)
    {
